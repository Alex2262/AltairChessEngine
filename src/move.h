...
public:
    ...
    Move(Square origin_square, Square target_square) : move(0) {
        move = (origin_square << 6) | target_square;
    }

    Move(Square origin_square, Square target_square, MoveType type) : move(0) {
        move = (type << 12) | (origin_square << 6) | target_square;
    }

    Move(Square origin_square, Square target_square, MoveType type, PromotionType promotion_type) : move(0) {
    ...
    [[nodiscard]] Square target() const { return Square(move & 0x3f); }
    [[nodiscard]] Square origin() const { return Square((move >> 6) & 0x3f); }
    [[nodiscard]] MoveType type() const { return MoveType((move >> 12) & 0x3); }
    [[nodiscard]] PromotionType promotion_type() const { return PromotionType((move >> 14) & 0x3); }
    [[nodiscard]] uint16_t internal_move() const { return move; };
...
private:
    ...
    uint32_t move;
public:
    InformativeMove() : move(0) {}

    explicit InformativeMove(uint32_t m) { move = m; }

    InformativeMove(Square origin, Square target, Piece selected, Piece occupied,
                           MoveType type, PromotionType promotion_type) : move(0) {
        move = (selected << 20) | (occupied << 16) | (promotion_type << 14) | (type << 12) | (origin << 6) | target;
    }

    InformativeMove(Move normal_move, Piece selected, Piece occupied) : move(0) {
    ...
    [[nodiscard]] MoveType type() const { return MoveType((move >> 12) & 0x3); }
    [[nodiscard]] PromotionType promotion_type() const { return PromotionType((move >> 14) & 0x3); }
    [[nodiscard]] Piece occupied() const { return Piece((move >> 16) & 0xf); }
    [[nodiscard]] Piece selected() const { return Piece((move >> 20) & 0xf); }
    [[nodiscard]] Move normal_move() const { return Move(move & 0xffff); }
