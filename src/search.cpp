...
void Engine::new_game() {
    ...
    for (Thread_State& thread_state : thread_states) {
        ...
        std::memset(thread_state.correction_history_ks, 0, sizeof(thread_state.correction_history_ks));
...
void Thread_State::update_correction_history_score(PLY_TYPE depth, SCORE_TYPE diff) {
    ...
    update_correction_history_entry(correction_history_ks[position.side][position.ks_hash_key % correction_history_size],
                                    depth, diff);
}
...
SCORE_TYPE Thread_State::correct_evaluation(SCORE_TYPE evaluation) {
    ...
    corrected += get_correction_score(correction_history_ks[position.side][position.ks_hash_key % correction_history_size]);
